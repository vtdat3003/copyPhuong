...
def get_arguments():
    ...
    args.add_argument('-w', '--weight_path', help='link to weight path',
                      default="./weights/yolov3-tiny_15000.weights")
    args.add_argument('-i', '--image_path', help='link to image file')
    args.add_argument('-cf', '--config_path', help='link to config path',
...
def draw_labels_and_boxes(image, labels, boxes):
    ...
    image = cv2.putText(image, labels, (x_min - 20, y_min), cv2.FONT_HERSHEY_SIMPLEX,
...
def convert2Square(image):
    ...
    :param image: input input_images
    :return: numpy array
    """
